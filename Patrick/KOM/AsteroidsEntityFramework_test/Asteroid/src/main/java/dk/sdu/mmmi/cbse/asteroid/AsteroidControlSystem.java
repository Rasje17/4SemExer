...
//            shapey[7] = (float) (y + Math.sin(radians + 3.1415f / 4) * 24);
        ...
        else if (asteroid.getSize().equals("MEDIUM")) {
            for (int i = 0; i < shapey.length; i++) {
                shapex[i] = (float) (x + Math.cos(radians + angle) * 16);
                shapey[i] = (float) (y + Math.sin(radians + angle) * 16);
                
                angle += 2*3.1415f / shapex.length;
            }
        }
        else if (asteroid.getSize().equals("SMALL")) {
            for (int i = 0; i < shapey.length; i++) {
                shapex[i] = (float) (x + Math.cos(radians + angle) * 8);
                shapey[i] = (float) (y + Math.sin(radians + angle) * 8);
