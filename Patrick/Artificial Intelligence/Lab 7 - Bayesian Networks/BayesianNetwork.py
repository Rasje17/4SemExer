...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        if len(self.parents) == 0:
            ...
            marginal_prob = []
            for asKey in self.assignments:
                summarize = 0
                for probKey in self.probability_table.keys():
                    multiplier = 1
                    for i in range(len(probKey)):
                        multiplier *= self.parents[i].get_marginal_probability(probKey[i])
                    multiplier *= self.probability_table[probKey][self.assignments.get(asKey)]
                    summarize += multiplier
                marginal_prob.append(summarize)
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        for key in values.keys():
            multi *= self.get_variable(key).get_marginal_probability(values[key])
...
def diagnostic():
    ...
    elecMalf = Variable('electronicMalfunction', ('false', 'true'), t2)
    fuelTankLeaking = Variable('fuelTankLeaking', ('false', 'true'), t3)
    vibrations = Variable('vibrations', ('false', 'true'), t4, [damagedTire])
    slowMaxSpeed = Variable('slowMaxSpeed', ('false', 'true'), t5, [damagedTire, elecMalf])
    highConsumption = Variable('highConsumption', ('false', 'true'), t6, [damagedTire, elecMalf, fuelTankLeaking])

    variables = [damagedTire, elecMalf, fuelTankLeaking, vibrations, slowMaxSpeed, highConsumption]
